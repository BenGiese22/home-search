...
| Launcher returns `{skipped:'in-progress'}` every time | a previous run left `started` without `done`. Since short-list #28 the launcher ignores a `started` older than the 3 h sandbox timeout, so this should not persist; before #28 it caused the 2026-09-08 → 09-19 outage (decisions, 2026-09-19) | reaper logs; `sandbox connect home-search-pipeline` → `cat data/.run/started` | launcher ignores a stale marker (#28) | if it persists, check that the marker's `started_at` is being read as epoch seconds (the Sep 8–19 outage was that mismatch) |
| Reap `stop-hung` about 10 min after a `started:true` launch | a regression of the seconds-vs-milliseconds marker bug (fixed in short-list `markers.ts`, 2026-09-23) | reaper logs: `stop-hung` 9–10 min after the launch | the run is killed mid-flight | check `markers.ts` and its contract test against real `run.py` bytes |
| Alert "<stage> partially failed" | a stage exited `EXIT_PARTIAL` (10): some items failed and the rest were written | the listing ids in the alert's log tail | later stages still run, and the failed items are retried next run | if the same ids recur every run, fix that listing's data |
| Launcher `skipped: locked` (bootstrap exit 75) | a run holds `data/.run/lock`, so bootstrap refused to reset its checkout | launcher logs | nothing; the next cron tries again | none, unless it persists past 3 h |
...
| Alert "FAILED at <stage>" ending in "looks transient" or "action needed" | any stage exited nonzero | the reason text in the alert itself: the failed stage's last 30 lines of log (#107) | nothing; the next run retries | if "action needed", read the tail. Until the child is run unbuffered, the tail can miss the traceback (see decisions 2026-09-19). Treat the transient/action verdict as a hint |
| "FAILED at commutes (exit 4)" | routed 0 of ≥ `NOTHING_ROUTED_FLOOR` (3) attempted: Mapbox outage/quota, or ≥ 3 permanently un-geocodable addresses re-picked every run | commutes log: one `no coordinates` / error line per listing | failed rows are retried next run | if the same addresses fail every run, they are bad addresses, not an outage; fix or exclude them |
| score_photos log "<id>: no longer in listings (rejected or delisted …" | a listing was rejected/deleted while its vision batch was in flight | stage log | result discarded; batch checkpoint cleared as normal | none |
